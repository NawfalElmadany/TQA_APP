...
const DeleteNoteConfirmationModal: React.FC<DeleteNoteConfirmationModalProps> = ({ isOpen, onClose, onConfirm, noteDate }) => {
  if (!isOpen) {
    return null;
  }
  
  const formattedDate = new Date(noteDate).toLocaleDateString('id-ID', {
    weekday: 'long',
    year: 'numeric',
    month: 'long',
    day: 'numeric',
    ...
    >
      ...
      >
        <div className="text-center">
          <h2 className="text-2xl font-bold mb-4 text-slate-900 dark:text-slate-100">Konfirmasi Hapus Catatan</h2>
          <p className="text-slate-600 dark:text-slate-400 mb-8">
            Apakah Anda yakin ingin menghapus catatan untuk tanggal <strong className="font-medium text-slate-800 dark:text-slate-100">{formattedDate}</strong>? Tindakan ini tidak dapat dibatalkan.
          </p>
        </div>
