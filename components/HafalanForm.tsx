...
import { getStudents, getClasses, addHafalanRecord, deleteLastHafalanRecord } from '../data/dataService';
...
const HafalanForm: React.FC = () => {
  ...
  const [ayatFrom, setAyatFrom] = useState<string>('');
  const [ayatTo, setAyatTo] = useState<string>('');
  const [score, setScore] = useState<string>('');
  const [notes, setNotes] = useState<string>('');
  const [date, setDate] = useState<string>(new Date().toISOString().split('T')[0]);
  const [errors, setErrors] = useState<{ selectedClass?: string; studentId?: string; surah?: string; ayatFrom?: string; ayatTo?: string; score?: string }>({});
  ...
  const handleVoiceResult = async (transcript: string) => {
    ...
    const ayatRangeMatch = lowerTranscript.match(/ayat\s+(\d+)\s+sampai\s+(\d+)/);
    const ayatSingleMatch = lowerTranscript.match(/ayat\s+(\d+)/);
    ...
    if (ayatRangeMatch) {
        setAyatFrom(ayatRangeMatch[1]);
        setAyatTo(ayatRangeMatch[2]);
    } else if (ayatSingleMatch) {
        setAyatFrom(ayatSingleMatch[1]);
        setAyatTo(ayatSingleMatch[1]);
    }
  ...
  const handleCorrectData = async () => {
    ...
    const [from, to] = lastSubmission.ayat.split('-');
    setAyatFrom(from);
    setAyatTo(to || from);
  ...
  const validate = (): boolean => {
    const newErrors: { selectedClass?: string; studentId?: string; surah?: string; ayatFrom?: string; ayatTo?: string; score?: string } = {};
    ...
    const ayatFromNum = Number(ayatFrom);
    const ayatToNum = Number(ayatTo);
    const selectedSurah = SURAHS.find(s => s.name === surah);

    // Ayat From validation
    if (!ayatFrom) {
        newErrors.ayatFrom = 'Ayat "dari" tidak boleh kosong.';
    } else if (isNaN(ayatFromNum) || !Number.isInteger(ayatFromNum) || ayatFromNum <= 0) {
        newErrors.ayatFrom = 'Ayat "dari" harus berupa angka bulat positif.';
    } else if (selectedSurah && ayatFromNum > selectedSurah.totalAyat) {
        newErrors.ayatFrom = `Ayat melebihi total ayat surat ini (${selectedSurah.totalAyat}).`;
    }

    // Ayat To validation (now optional)
    if (ayatTo) {
      if (isNaN(ayatToNum) || !Number.isInteger(ayatToNum) || ayatToNum <= 0) {
          newErrors.ayatTo = 'Ayat "sampai" harus berupa angka bulat positif.';
      } else if (selectedSurah && ayatToNum > selectedSurah.totalAyat) {
          newErrors.ayatTo = `Ayat melebihi total ayat surat ini (${selectedSurah.totalAyat}).`;
      } else if (!newErrors.ayatFrom && ayatToNum < ayatFromNum) {
          newErrors.ayatTo = 'Ayat "sampai" tidak boleh kurang dari ayat "dari".';
      }
    }
  ...
  return (
    <form onSubmit={handleSubmit} ref={formRef}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <FormGroup label="Ayat Dari">
                <Input type="number" placeholder="Contoh: 1" value={ayatFrom} onChange={(e) => setAyatFrom(e.target.value)} required />
                {errors.ayatFrom && <ErrorMessage>{errors.ayatFrom}</ErrorMessage>}
            </FormGroup>

            <FormGroup label="Ayat Sampai">
                <Input type="number" placeholder="Sampai (Ops.)" value={ayatTo} onChange={(e) => setAyatTo(e.target.value)} />
                {errors.ayatTo && <ErrorMessage>{errors.ayatTo}</ErrorMessage>}
            </FormGroup>
        ...
        <FormGroup label="Catatan Guru">
            <TextArea rows={4} placeholder="Contoh: Tajwid perlu diperbaiki di beberapa bagian..." value={notes} onChange={(e) => setNotes(e.target.value)} />
        </FormGroup>
