    ...
    <li
      ...
      className={`flex items-center space-x-4 p-3 rounded-lg cursor-pointer transition-all duration-200 relative ${
        isActive
          ...
          : 'text-slate-500 dark:text-slate-400 hover:bg-dark-border hover:text-slate-100'
      }`}
    >
      ...
      {badgeCount !== undefined && badgeCount > 0 && (
        <span className="ml-auto bg-brand-cyan text-black text-xs font-bold w-5 h-5 rounded-full flex items-center justify-center animate-fade-in">
...
const Sidebar: React.FC<SidebarProps> = ({ activePage, setActivePage, remindersCount }) => {
  return (
    <>
      <aside className="w-full md:w-64 bg-dark-card border-r border-dark-border p-4 md:p-6 flex-shrink-0 flex flex-col justify-between">
        <div>
          <div className="flex items-center space-x-3 mb-10">
            <Logo className="w-10 h-10" />
            <h1 className="text-xl font-bold text-slate-100">TQA App</h1>
          </div>
          <nav>
