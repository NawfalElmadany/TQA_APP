        ...
        <div className="overflow-x-auto max-h-96">
            <table className="w-full text-left">
                <thead className="bg-slate-800/50">
                    <tr>
                        {headers.map(header => <th key={header} className="p-3 text-sm font-semibold text-slate-300 uppercase tracking-wider">{header}</th>)}
                    </tr>
                </thead>
                <tbody className="divide-y divide-dark-border">
                    {data.length > 0 ? data.map((row, rowIndex) => (
                        <tr key={rowIndex} className="hover:bg-slate-800/50 transition-colors">
                            {row.map((cell, cellIndex) => <td key={cellIndex} className="p-3 text-sm text-slate-300">{cell}</td>)}
                        </tr>
                    )) : (
                        <tr>
                            <td colSpan={headers.length} className="text-center p-4 text-slate-400">Belum ada riwayat.</td>
                        </tr>
                    )}
                </tbody>
            </table>
...
const StatusBadge: React.FC<{ status: HafalanStatus }> = ({ status }) => {
    const statusStyles = {
        ...
        'Belum Dimulai': 'bg-slate-700 text-slate-300',
    };
    return (
    ...
    return (
        <div className="bg-slate-800/50 p-4 rounded-lg border border-dark-border">
            <div className="flex justify-between items-center mb-2">
                <h4 className="font-semibold text-slate-100">{surah.name}</h4>
                <StatusBadge status={surah.status} />
            </div>
            <div className="w-full bg-slate-700 rounded-full h-2">
                <div className="bg-brand-accent h-2 rounded-full" style={{ width: `${progress}%` }}></div>
            </div>
            <p className="text-right text-xs mt-1.5 text-slate-400">{surah.memorizedAyat}/{surah.totalAyat} Ayat</p>
        </div>
    );
};
...
const StudentDashboard: React.FC<StudentDashboardProps> = ({ studentId, onLogout }) => {
  ...
  if (!studentData) {
    return (
      <div className="min-h-screen flex items-center justify-center text-center animate-fade-in text-slate-200">
        <div>
  ...
  return (
    <>
      <div className="min-h-screen bg-dark-background text-slate-200 font-sans p-4 md:p-8">
          <div className="max-w-7xl mx-auto space-y-8 animate-fade-in">
              <header className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                  <div className="flex items-center space-x-4">
                      <div className="relative group cursor-pointer" onClick={() => setIsProfilePicModalOpen(true)}>
                          {profilePic ? (
                              ...
                              <div className="w-16 h-16 rounded-full bg-slate-700 border-2 border-slate-600 flex items-center justify-center">
                                  <Icon name="profil" className="w-8 h-8 text-slate-400" />
                              </div>
                          )}
                      ...
                      <div>
                          <h2 className="text-3xl font-bold text-slate-50">{name}</h2>
                          <p className="text-slate-400">Dashboard Siswa</p>
                      </div>
                  </div>
                  <button
                      onClick={onLogout}
                      className="flex items-center space-x-2 bg-red-500/20 text-red-400 font-medium py-2 px-4 rounded-lg hover:bg-red-500/30 hover:text-red-300 transition-colors duration-200"
                  >
              ...
              <Card>
                  <h3 className="text-xl font-semibold mb-4 text-slate-100">Pencapaian Hafalan Terakhir</h3>
                  {lastAchievedSurah ? (
                      <SurahTargetCard surah={lastAchievedSurah} />
                  ) : (
                      <p className="text-slate-400 text-center py-4">Belum ada pencapaian hafalan yang tercatat.</p>
                  )}
              </Card>

              <Card>
                  <h3 className="text-xl font-semibold mb-4 text-slate-100">Grafik Nilai Perkembangan</h3>
                  <div className="h-80">
